...
% strc{1}=[1,2,15,16];    strc{2}=[3,4,5,6];  strc{3}=[3,4,5,6];  strc{4}=[7,8,9,10,11,12,13,14];
...
x=zeros(n,1);
x0=x;
u0= K*(x0) + noise*noise_value*randn(m,1)+disturbance;
x = A*x0 + B*(u0);
...
%         end
%         u0= K*x0 + noise*noise_val(i)*richnoiseN(i,m,noise_bias)+disturbance;
%         u0= K*x0 + noise*noise_val(i)*randn(m,1)+disturbance;
        u0= K*(x0) + noise*noise_value*randn(m,1)+disturbance;
        x = A*x0 + B*(u0);
        xxx=[xxx x0];
        uuu=[uuu u0];
