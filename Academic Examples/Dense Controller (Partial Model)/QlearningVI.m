function [K] = sparsefunc(A,B,Q,R,states,inputs)
...
upd_thrsh = 5e-1; %threshhold before updating
...
for k=1:samples
    ...
    Phi = phi0-gamma*phi;
    W0 = W{sw};
    r = x0(states{sw})'*Qi{sw}*x0(states{sw}) + u(inputs{sw})'*Ri{sw}*u(inputs{sw})+W0'*phi;
    S0=S{sw};
    [W{sw}, S{sw}] = RLS(phi0,r,W0,S0);
    WW{sw} = [WW{sw} W{sw}];
    j{sw}=j{sw}+1;
